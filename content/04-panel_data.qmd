...
-   This [UCLA Statistical Methods and Data Analytics FAQ](https://stats.oarc.ucla.edu/sas/faq/how-can-i-interpret-log-transformed-variables-in-terms-of-percent-change-in-linear-regression/).

-   This [StackExchange thread](https://stats.stackexchange.com/questions/18480/interpretation-of-log-transformed-predictor-and-or-response/18639#18639).
:::
...
I then find the predicted increase in a country's life expectancy resulting from a \$1,000 (or a `r round(log(1000), 3)` logged dollar) increase in a country's GDP per capita, holding the yearly effects constant:

```{r}
coef_gdp_linear * log(1000)
```

Holding a country's GDP per capita constant, this average life expectancy increases by an average of `r round(tidy(m_linear) |> filter(term == "year") |> pull(estimate), 2)` years annually.
...
We also have the country-specific effects. We find that life expectancy varies by `r tidy(m_multi) |> filter(term == "sd__(Intercept)") |> pull(estimate) |> round(3)` years between countries. This country-level variance accounts for `r scales::percent((tidy(m_multi) |> filter(term == "sd__(Intercept)") |> pull(estimate)) / ((tidy(m_multi) |> filter(term == "sd__(Intercept)") |> pull(estimate)) +  (tidy(m_multi) |> filter(term == "sd__Observation") |> pull(estimate))))` of the total residual variation of the model. How did I get that?
...
-   This lab session draws heavily on Andrew Heiss's brilliant blog post [*A guide to working with country-year panel data and Bayesian multilevel models* (2021)](https://www.andrewheiss.com/blog/2021/12/01/multilevel-models-panel-data-guide/).

-   I also highly recommend Michael Clark's [*Mixed Models with R*](https://m-clark.github.io/mixed-models-with-R/).
