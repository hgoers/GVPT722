...
They provide us with data on `r nrow(ebj_raw)` post-conflict states. We are going to clean this up a little for our purposes today. We will be concentrating on four variables:

-   `net_fdi`: net FDI inflows over a 10-year period after a conflict (USD, millions);

-   `truthvictim`: whether the state established a post-conflict justice institution after the conflict;

-   `gdp_per_cap`: the country's annual GDP per capita (USD); and

-   `polity2`: the country's regime type (which we will convert into a categorical variable listing whether the country is a democracy, autocracy, or hybrid regime).
...
Appel and Loyle are interested in measuring the relationship between net FDI inflows over a 10-year period after a conflict and the establishment of a truth and reconciliation commission. Let's take a quick look at these two main variables of interest. 
...
```{r}
summary(ebj_df$net_fdi)
```

::: {.callout-note}
There appears to be a clear outlier: a net inflow of `r ebj_df |> slice_max(net_fdi) |> pull(net_fdi) |> scales::dollar()` million. You can see that this is pulling up the average net FDI inflow well above the median inflow. We will keep this in because this is not the focus of today's session, but I would encourage you to explore whether these findings are sensitive to its inclusion. 
:::
...
Post-conflict states that did not establish a truth and reconciliation commission received, on average, net FDI inflows of `r tidy(m) |> filter(term == "(Intercept)") |> pull(estimate) |> scales::dollar()` million in the 10-year period after conflict. 
:::
...
$$
Average\ net\ FDI\ inflows = \beta_0 + \beta_1 Commission\ established + \epsilon
$$

When a commission was established (i.e. $Commission\ established = 1$):
...
We can see the positive relationship between GDP per capita and net FDI inflows *and* that states that have established a commission start and stay at a higher predicted inflow than those that did not establish a commission for any given value of GDP per capita. 
